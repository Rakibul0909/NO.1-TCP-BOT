...
from Plugins import DEcwHisPErMsG_pb2 , MajoRLoGinrEs_pb2 , PorTs_pb2 , MajoRLoGinrEq_pb2 , sQ_pb2 , Team_msg_pb2, RemoveFriend_Req_pb2, GetFriend_Res_pb2, spam_request_pb2, devxt_count_pb2, dev_generator_pb2, kyro_title_pb2, room_join_pb2
...
server2 = "ind"
key2 = "nfe69"
BYPASS_TOKEN = "NFE-NAJME-ALAM-TOKENS-SJDJSJ72NDJE83"
WHITELIST_ONLY = False
bot_enabled = True
BOT_OWNER_UID = 2050444909  
PLAYER_NAME_CACHE = {}  
...
📱 Player will see:
"Rakibul wants to join your team"
...
async def black666(client_id, key, iv):
    banner_text = "[FF0000][B][C] ERROR , WELCOME TO [FFFFFF]Rakibul [00FF00]___ Rakibul____ BOT ! \n[FFFF00]NEW VERSION NEW FUNCTION !\n[FF0000]TELEGRAM : @Rakibul\n\n"     
    fields = {
        1: 5,
        2: {
            1: int(client_id),
...
Region: {server_name}
[FFDD00]°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°
[00FF00][b][c]BOT MADE BY Rakibul 
"""
            else:
...
def send_likes(uid):
    try:
        likes_api_response = requests.get(
             f"https://like-api.vercel.app/like?uid={uid}&server_name=bd",
             timeout=15
             )
...
[C][B][11EAFD]‎━━━━━━━━━━━━
[C][B][FFB300]Subscribe: [FFFFFF]Rakibul [00FF00]!!
"""
...
async def RejectMSGtaxt(squad_owner,uid, key, iv):
    random_banner = f"""[00FF00]WELCOME_TO_MG24_TCP_BOT
    [FF0000]HOW_ARE_YOU?"""
    fields = {
    1: 5,
    2: {
...
"""
            ...
            # For example:
            title_packet = await convert_kyro_to_your_system(uid, chat_id, key, iv, nickname="Rakibul", title_id=title_id)
...
async def noob(target_uid, chat_id, key, iv, nickname="Rakibul", title_id=None):
    """EXACT conversion with customizable title ID"""
    try:
        # Use provided title_id or get random one
        if title_id is None:
...
async def send_all_titles_sequentiallly(uid, chat_id, key, iv, region, chat_type):
    ...
    # Send initial message
    start_msg = f"""[B][C][00FF00] Noobde Rakibul ya meku agar tu noob bolra toh tu g a y hai
...
"""
    ...
    try:
        for index, title_id in enumerate(all_titles):
            ...
            # You'll need to use your existing title sending logic here
            # For example:
            title_packet = await noob(uid, chat_id, key, iv, nickname="Rakibul", title_id=title_id)
              ...
              # =================== EMOTE DETECTION ONLY ===================
                    ...
                    try:
                        ...
                        if "NM" in WHITELISTED_UIDS:
                            ...
                            try:
                                ...
                                bot_uid = 13777711848
                                message_text = f" Can't accept Your request Talk to Rakibul"
                ...
                if insquad == True:
                    try:
                        ...
                        message = """[B][C][FF0000]█▓▒░░ WELCOME TO Rakibul BOT ░░▒▓█
[00FF00]»»————-　★　————-««
[FFFFFF]⚡ BOT POWERED BY:[FF0000] Rakibul
[FFFFFF]🎯 STATUS: [00FF00]ACTIVE 24/7
[FFFFFF]📱 CONTACT: [FFFF00]@Rakibul
[00FF00]»»————-　★　————-««
...
[b][i][FFC0CB]youtube: Rakibul 99[/b]

[b][c][FFC0CB]subcribe: my_channel[FFFFFF]
 
[b][i][FFA500]telegram: @Rakibul[/b]

[b][c][FFA500]telegram contact: @Rakibul[A52A2A]
...
#==================≈===========  /HELP MENU COMMANDS ========================================
                        if inPuTMsG.strip().lower() in ("help", "/help", "menu", "hello", "hi"):
                            ...
                            header = f"[C][B][FF1493]Hey User Welcome To [00FFAA]Rakibul's BOT"
                            await safe_send_message(response.Data.chat_type, header, uid, chat_id, key, iv)
                            await asyncio.sleep(0.2)

                            menu1 = """[FFD700]═══⚡ TEAM & BASIC ⚡═══
...
async def MaiiiinE():
    ...
    if not credentials:
        ...
        print("📝 Format: uid=4634566383,password=E10F3E16A59B5D15FBF806555E4FCB27AC8AD2242E464FDB130B09596A9D42C0")
        return None
    ...
    os.system('clear')
    print("=" * 50)
    print("🤖 Rakibul BOT - INITIALIZING")
    print("=" * 50)
